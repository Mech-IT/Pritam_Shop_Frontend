...
const Forgotpassword = () => {
    ...
    const handleFpassword = async (e) => {
        ...
        if (!ref.current.checkValidity()) {
            ...
            document.getElementById('loaderF').style.display = 'flex'
            document.getElementById('mainF').style.opacity = 0.5
            // const host = "http://localhost:5000"
            const host = process.env.REACT_APP_BASE_URL 
            const response = await fetch(`${host}/api/users/fpass`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'auth-token': stayUser === true ? localStorage.getItem("token") : sessionStorage.getItem("token")
                },
                body: JSON.stringify({email:payload.user.identifier,password:user.password})
            })
            if (response.status >= 200 && response.status <= 299) {
                document.getElementById('loaderF').style.display = 'none'
                setResult("success")
                document.getElementById('alertF').style.display = 'block'
                setTimeout(() => {
                    document.getElementById('alertF').style.display = 'none'
                    navigate("/login")
                }, 3000);
            } else {
                ...
                console.log(json)
                document.getElementById('loaderF').style.display = 'none'
                document.getElementById('mainF').style.opacity = 1;
                setResult("error")
                document.getElementById('alertF').style.display = 'block'
                setTimeout(() => {
                    document.getElementById('alertF').style.display = 'none'
        ...
        <>
            <Container>
                ...
                <pre style={{ display: "flex", justifyContent: "center", alignItems: "center", backgroundColor: "gray" }}>
                    <div className='container my-2 ' id='mainF' style={{margin:"5px"}}>
                        <h2 className='text-center my-5' >Welcome To Pritam-Shop</h2>
                        <h3 className='text-center my-5'>Forgot Password</h3>
                        <form className='needs-validation' ref={ref} onSubmit={handleFpassword} noValidate oninput='confirmPassword.setCustomValidity(confirmPassword.value != newPassword.value ? true : false)'>
